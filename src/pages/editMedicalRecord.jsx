...
export default function MedicalRecords() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:4000/api/users/get-records", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await toast.promise(
          axios.put(`http://localhost:4000/api/users/edit-record/${id}`, formData, {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          }),
          {
        ...
        await toast.promise(
          axios.post("http://localhost:4000/api/users/create-record", formData, {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          }),
          {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await axios.delete(`http://localhost:4000/api/users/delete-record/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      });
      if (res.data.success) {
